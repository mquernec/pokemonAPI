...
{
    private readonly List<string> _pokemonList;
    private readonly List<Pokemon> _customPokemon;

    public PokemonRepository()
    {
        _customPokemon = new List<Pokemon>();
        _pokemonList= new List<string>
            {
                // Génération 1
                ...
                "Whirlipede", "Scolipede", "Cottonee", "Whimsicott", "Petilil", "Lilligant", "Basculin", "Sandile", "Krokorok", "Krookodile",
                ...
                "Zweilous", "Hydreigon", "Larvesta", "Volcarona", "Cobalion", "Terrakion", "Virizion", "Tornadus", "Thundurus", "Reshiram",
                ...
                "Coalossal", "Applin", "Flapple", "Appletun", "Silicobra", "Sandaconda", "Cramorant", "Arrokuda", "Barraskewda", "Toxel",
                ...
                "Alcremie", "Falinks", "Pincurchin", "Snom", "Frosmoth", "Stonjourner", "Eiscue", "Indeedee", "Morpeko", "Cufant",
                "Copperajah", "Dracozolt", "Arctozolt", "Dracovish", "Arctovish", "Duraludon", "Dreepy", "Drakloak", "Dragapult", "Zacian",
                ...
                "Maushold", "Squawkabilly", "Nacli", "Naclstack", "Garganacl", "Charcadet", "Armarouge", "Ceruledge", "Tadbulb", "Bellibolt",
                "Wattrel", "Kilowattrel", "Maschiff", "Mabosstiff", "Shroodle", "Grafaiai", "Bramblin", "Brambleghast", "Toedscool", "Toedscruel",
                "Klawf", "Capsakid", "Scovillain", "Rellor", "Rabsca", "Flittle", "Espathra", "Tinkatink", "Tinkatuff", "Tinkaton",
                "Wiglett", "Wugtrio", "Bombirdier", "Finizen", "Palafin", "Varoom", "Revavroom", "Cyclizar", "Orthworm", "Glimmet",
                "Glimmora", "Greavard", "Houndstone", "Flamigo", "Cetoddle", "Cetitan", "Veluza", "Dondozo", "Tatsugiri", "Annihilape",
                "Clodsire", "Farigiraf", "Dudunsparce", "Kingambit", "Great Tusk", "Scream Tail", "Brute Bonnet", "Flutter Mane", "Slither Wing", "Sandy Shocks",
                "Iron Treads", "Iron Bundle", "Iron Hands", "Iron Jugulis", "Iron Moth", "Iron Thorns", "Frigibax", "Arctibax", "Baxcalibur", "Gimmighoul",
                "Gholdengo", "Wo-Chien", "Chien-Pao", "Ting-Lu", "Chi-Yu", "Roaring Moon", "Iron Valiant", "Koraidon", "Miraidon", "Walking Wake",
                "Iron Leaves", "Dipplin", "Poltchageist", "Sinistcha", "Okidogi", "Munkidori", "Fezandipiti", "Ogerpon", "Archaludon", "Hydrapple",
                "Gouging Fire", "Raging Bolt", "Iron Boulder", "Iron Crown", "Terapagos"
            };
    }    public IReadOnlyCollection<Pokemon> Get()
    {
        var defaultPokemon = _pokemonList
            .Select((name, index) => new Pokemon(
                id: index + 1,
                name: name,
                type: "Unknown", // Type par défaut, pourrait être enrichi plus tard
                level: 1, // Niveau par défaut
                ability: "Unknown" // Capacité par défaut, pourrait être enrichie plus tard
            ));

        return defaultPokemon.Concat(_customPokemon).ToList().AsReadOnly();
    }
    ...
    {
        if (pokemon != null && !_customPokemon.Any(p => p.Id == pokemon.Id))
        {
            // Assigner un nouvel ID basé sur le nombre total de Pokémon
            var maxId = Get().Any() ? Get().Max(p => p.Id) : 0;
            pokemon.Id = maxId + 1;
    ...
    public void Update(Pokemon pokemon)
    {
        ...
        var existingCustomPokemon = _customPokemon.FirstOrDefault(p => p.Id == pokemon.Id);
        if (existingCustomPokemon != null)
        {
            var index = _customPokemon.IndexOf(existingCustomPokemon);
            _customPokemon[index] = pokemon;
        }
        else
        {
            // Si c'est un Pokémon par défaut, on le copie dans les customs pour pouvoir le modifier
            var defaultPokemon = Get().FirstOrDefault(p => p.Id == pokemon.Id);
    ...
    {
        var pokemon = _customPokemon.FirstOrDefault(p => p.Id == id);
        if (pokemon != null)
        {
            _customPokemon.Remove(pokemon);
        }
    }
}
